...
if __name__ == '__main__':
	...
	parser.add_argument('-f', dest='output_path', type=str, default='../dataset4/', help='Path to output files.')
	parser.add_argument('-l', dest='gen_lr', type=bool, default=False, help='Generate low resolution (LR) counterpart images')
	parser.add_argument('-bic', dest='gen_bic', type=bool, default=False, help='Generate bicubic upsampled images')
	...
	for key in datasets.keys():
		for i in range(datasets[key]):
			...
			# Generate an upsampled image from bicubic interpolation (4x resolution)
			if args.gen_bic:
				img_bic = img_hr.resize((int(args.sample_size*args.upscale),
			...
			if args.gen_lr:
				...
				with open(filename,'wb') as fh:
					img_lr.save(fh)
			if args.gen_bic:
				filename = args.output_path+key+'/{:04d}_bic.png'.format(i)
				with open(filename,'wb') as fh:
					img_bic.save(fh)
